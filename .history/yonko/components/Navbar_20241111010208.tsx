...
const categoryMappings = {
  tech: {
    ...
    properties: {
      name: "name",
      image: "images",
      location: "shopId", // Assuming you want to use shopId as the location identifier
    },
  },
  clothing: {
...
const Navbar: React.FC = () => {
  ...
  const handlePredictionClick = async (item) => {
    ...
    const categoryMapping = categoryMappings[item.category.toLowerCase()];

    if (!categoryMapping) {
      console.error("Category mapping not found for category:", item.category);
      return;
    }

    const { screen, properties } = categoryMapping;
    ...
    navigation.navigate(screen, {
      ...
      restaurantName: store.name, // Add any additional required parameters
      restaurantImageUri: store.avatar, // Add any additional required parameters
      restaurantLocation: store.address, // Add any additional required parameters
  ...
  const fetchStoreById = async (shopId) => {
    try {
      ...
      if (response.data && response.data.shop) {
        console.log("Fetched store data:", response.data.shop); // Log the store data
