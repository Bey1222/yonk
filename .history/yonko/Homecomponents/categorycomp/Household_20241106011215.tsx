...
const Household = () => {
  ...
  const bestSellers = [
    { name: "Cleaning Supplies", image: require("../../assets/all9.jpg") },
    { name: "Kitchenware", image: require("../../assets/all5.jpg") },
    { name: "Bed & Bath", image: require("../../assets/all12.jpg") },
    { name: "Home Decor", image: require("../../assets/all18.jpg") },
  ];
    ...
    const brandItems = categoryData.flatMap((item) =>
      item.menu
        ...
        .map((menuItem) => ({
          ...
          householdImageUri: item.image,
          householdLocation: item.location,
        }))
    );

    if (brand === "Cleaning Supplies") {
      navigation.navigate("CleaningSuppliesScreen", { items: brandItems });
    } else if (brand === "Kitchenware") {
      navigation.navigate("KitchenwareScreen", { items: brandItems });
    } else if (brand === "Bed & Bath") {
      navigation.navigate("BedBathScreen", { items: brandItems });
    } else if (brand === "Home Decor") {
      navigation.navigate("HomeDecorScreen", { items: brandItems });
    }
  };
  ...
  const handleNavigation = (item, screenName) => {
    ...
    navigation.navigate(screenName, {
      household,
      householdName: store.name,
      householdImageUri: store.avatar,
      householdLocation: `${store.address.street}, ${store.address.city}, ${store.address.state}, ${store.address.country}`,
    });
  };

  const handleNavigationHouseholdProducts = (item, screenName) => {
    ...
    if (screenName === "HouseholdProducts") {
      console.log("Navigating to HouseholdProducts with item:", item);
      navigation.navigate(screenName, {
        item,
        householdName: store.name,
        householdImageUri: store.avatar,
  ...
  return (
    <LinearGradient
      colors={["#737373", "rgba(175, 238, 238, .5)", "#737373"]}
      locations={[1, 0.2, 1]}
      start={{ x: 0.5, y: 0 }}
      end={{ x: 0, y: 0.7 }}
            ...
            <View style={styles.sectionHeader}>
              ...
              {section !== "Best Sellers" && (
                <TouchableOpacity
                  ...
                  onPress={() => {
                    if (section === "Essential Items") {
                      navigation.navigate("EssentialItems", {
                        essentialItems: Object.values(storeData),
                      });
                    } else if (section === "Discounts") {
                      navigation.navigate("DiscountsHousehold", {
                        discountItems: categoryData,
                      });
                    } else if (section === "New Arrivals") {
                      navigation.navigate("NewarrivalsHousehold", {
                ...
                {sectionData[section]?.map((item, idx) => {
                  ...
                  return (
                    <TouchableOpacity
                      ...
                      style={styles.card}
                      onPress={() =>
                        handleNavigationHouseholdProducts(
                          item,
                          "HouseholdProducts"
                        )
                      }
                ...
                {sectionData[section]?.map((store, idx) => {
                  ...
                  return (
                    <TouchableOpacity
                      ...
                      style={styles.sectionItem}
                      onPress={() =>
                        handleNavigation(store, "MainstoreHousehold")
                      }
                    >
                      <View style={styles.imageContainer}>
                        <Image
