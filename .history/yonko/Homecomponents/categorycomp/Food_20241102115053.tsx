...
const Food = () => {
  ...
  const fetchAllStores = async () => {
    try {
      ...
      const data = await response.json();
      console.log("Fetched all stores data:", data); // Log the fetched data
      if (data && data.shops) {
        const storeMap = {};
        data.shops.forEach((shop) => {
          storeMap[shop.id] = shop;
        });
            ...
            {section === "Discounts" ? (
              <View style={styles.listContainer}>
                {sortItems(sectionData[section]).map((item, idx) => {
                  ...
                  console.log("Discounts item:", item, "Store:", store); // Log the item and store data
                  return (
                    <TouchableOpacity
                      key={idx}
                      style={styles.card}
              ...
              >
                {sortItems(sectionData[section]).map((item, idx) => {
                  const store = storeData[item.shopId];
                  console.log("New Arrivals item:", item, "Store:", store); // Log the item and store data
                  return (
                    <TouchableOpacity
                      key={idx}
