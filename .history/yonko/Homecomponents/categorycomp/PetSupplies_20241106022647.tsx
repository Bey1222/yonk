...
const PetSupplies = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          `${process.env.EXPO_PUBLIC_BASE_URL}/products?category=PET`,
          {
            headers: {
  ...
  const bestSellers = [
    { name: "Dog Food", image: require("../../assets/all24.jpg") },
    { name: "Cat Food", image: require("../../assets/all17.jpg") },
    { name: "Pet Toys", image: require("../../assets/all24.jpg") },
    { name: "Pet Accessories", image: require("../../assets/all22.jpg") },
  ];
  ...
  const navigateToBrandScreen = (brand) => {
    const brandItems = categoryData.flatMap((item) =>
      item.menu
        ...
        .map((menuItem) => ({
          ...menuItem,
          petSupplyName: item.name,
          petSupplyImageUri: item.image,
          petSupplyLocation: item.location,
    ...
    if (brand === "Dog Food") {
      navigation.navigate("DogFoodScreen", { items: brandItems });
    } else if (brand === "Cat Food") {
      navigation.navigate("CatFoodScreen", { items: brandItems });
    } else if (brand === "Pet Toys") {
      navigation.navigate("PetToysScreen", { items: brandItems });
    } else if (brand === "Pet Accessories") {
      navigation.navigate("PetAccessoriesScreen", { items: brandItems });
    }
  };
  ...
  const handleNavigation = (item, screenName) => {
    ...
    navigation.navigate(screenName, {
      petSupply,
      petSupplyName: store.name,
      petSupplyImageUri: store.avatar,
      petSupplyLocation: `${store.address.street}, ${store.address.city}, ${store.address.state}, ${store.address.country}`,
    });
  };

  const handleNavigationPetSuppliesProducts = (item, screenName) => {
    ...
    if (screenName === "PetSuppliesProducts") {
      console.log("Navigating to PetSuppliesProducts with item:", item);
      navigation.navigate(screenName, {
        item,
        petSupplyName: store.name,
        petSupplyImageUri: store.avatar,
  ...
  return (
    <LinearGradient
      colors={["#737373", "rgba(255, 240, 245, .4)", "#737373"]}
      locations={[1, 0.2, 1]}
      start={{ x: 0.5, y: 0 }}
      end={{ x: 0, y: 0.7 }}
                  ...
                  onPress={() => {
                    if (section === "Pet Care") {
                      navigation.navigate("PetCare", {
                        petCare: Object.values(storeData),
                      });
                    } else if (section === "Discounts") {
                      navigation.navigate("DiscountsPetSupplies", {
                        discountItems: categoryData,
                      });
                    } else if (section === "New Arrivals") {
                      navigation.navigate("NewarrivalsPetSupplies", {
            ...
            {section === "Discounts" ? (
                ...
                {sortItems(sectionData[section]).map((item, idx) => {
                  ...
                  return (
                    <TouchableOpacity
                      ...
                      style={styles.card}
                      onPress={() =>
                        handleNavigationPetSuppliesProducts(
                          item,
                          "PetSuppliesProducts"
                        )
                      }
              ...
              >
                {sortItems(sectionData[section]).map((store, idx) => {
                  ...
                  console.log("Pet Care item:", store, "Products:", products);
                  return (
                    <TouchableOpacity
                      key={idx}
                      style={styles.sectionItem}
                      onPress={() =>
                        handleNavigation(store, "MainstorePetSupplies")
                      }
                    >
                      <View style={styles.imageContainer}>
                        <Image
