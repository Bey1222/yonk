...
const SpaBeauty = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          `${process.env.EXPO_PUBLIC_BASE_URL}/products?category=SPA_BEAUTY`,
          {
            headers: {
  ...
  const handleItemPress = (item, section) => {
    if (section === "Relaxing Spas") {
      navigation.navigate("MainstoreSpaBeauty", { spa: item });
    } else if (section === "Discounts" || section === "New Arrivals") {
      navigation.navigate("SpaBeautyProducts", {
        item,
        spaName: item.spaName,
        spaImageUri: item.spaImageUri,
        spaLocation: item.spaLocation,
      });
    }
  };
