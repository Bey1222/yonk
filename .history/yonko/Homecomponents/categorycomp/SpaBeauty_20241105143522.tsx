...
const SpaBeauty = () => {
  ...
  const bestSellers = [
    { name: "Skincare", image: require("../../assets/all16.jpg") },
    { name: "Makeup", image: require("../../assets/makeup.jpg") },
    { name: "Hair Care", image: require("../../assets/all21.jpg") },
    { name: "Nail Care", image: require("../../assets/all0.jpg") },
  ];
    ...
    const brandItems = categoryData.flatMap((item) =>
      item.menu
        ...
        .map((menuItem) => ({
          ...menuItem,
          spaName: item.name,
          spaImageUri: item.image,
          spaLocation: item.location,
        }))
    );

    if (brand === "Skincare") {
      navigation.navigate("SkincareScreen", { items: brandItems });
    } else if (brand === "Makeup") {
      navigation.navigate("MakeupScreen", { items: brandItems });
    } else if (brand === "Hair Care") {
      navigation.navigate("HairCareScreen", { items: brandItems });
    } else if (brand === "Nail Care") {
      navigation.navigate("NailCareScreen", { items: brandItems });
    }
  };
  ...
  const handleNavigation = (item, screenName) => {
    ...
    navigation.navigate(screenName, {
      spa,
      spaName: store.name,
      spaImageUri: store.avatar,
      spaLocation: `${store.address.street}, ${store.address.city}, ${store.address.state}, ${store.address.country}`,
    });
  };

  const handleNavigationSpaProducts = (item, screenName) => {
    ...
    if (screenName === "SpaBeautyProducts") {
      console.log("Navigating to SpaBeautyProducts with item:", item);
      navigation.navigate(screenName, {
        item,
        spaName: store.name,
        spaImageUri: store.avatar,
  ...
  return (
    <LinearGradient
      colors={["#737373", "rgba(241,175,189,.5)", "#737373"]}
      locations={[1, 0.2, 1]}
      start={{ x: 0.5, y: 0 }}
      end={{ x: 0, y: 0.7 }}
            ...
            <View style={styles.sectionHeader}>
              ...
              {section !== "Best Sellers" && (
                <TouchableOpacity
                  ...
                  onPress={() => {
                    if (section === "Relaxing Spas") {
                      navigation.navigate("RelaxingSpas", {
                        relaxingSpas: Object.values(storeData),
                      });
                    } else if (section === "Discounts") {
                      navigation.navigate("DiscountSpaBeauty", {
                        discountItems: categoryData,
                      });
                    } else if (section === "New Arrivals") {
                      navigation.navigate("NewarrivalsSpaBeauty", {
                ...
                {sectionData[section]?.map((item, idx) => {
                  ...
                  return (
                    <TouchableOpacity
                      ...
                      style={styles.card}
                      onPress={() =>
                        handleNavigationSpaProducts(item, "SpaBeautyProducts")
                      }
                    >
                ...
                {sectionData[section]?.map((store, idx) => {
                  ...
                  return (
                    <TouchableOpacity
                      ...
                      style={styles.sectionItem}
                      onPress={() =>
                        handleNavigation(store, "MainstoreSpaBeauty")
                      }
                    >
                      <View style={styles.imageContainer}>
                        <Image
