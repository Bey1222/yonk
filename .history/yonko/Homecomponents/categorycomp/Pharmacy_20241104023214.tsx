    ...
    if (categoryData && Object.keys(storeData).length > 0) {
      ...
      sections.forEach((section, index) => {
        if (section === "Top Pharmacies") {
          const filteredStores = Object.values(storeData).filter((store) =>
            categoryData.some((product) => product.shopId === store.id)
          );
          data[section] = shuffleArray(filteredStores).slice(0, 5);
  ...
  const handleNavigation = (item, screenName) => {
    ...
    if (screenName === "MainstorePharmacy") {
      navigation.navigate(screenName, {
        pharmacy: {
          shop: item,
          products: item.products, // Pass the products to MainstorePharmacy
        },
      });
    } else {
      navigation.navigate(screenName, {
        item: {
          ...
          pharmacyLocation: store.address,
          isOpen: item.isOpen,
          openingTime: item["Opening Time"],
          closingTime: item["Closing Time"],
        },
            ...
            {section === "Discounts" ? (
              ...
              >
                {sortItems(sectionData[section]).map((store, idx) => {
                  ...
                  return null; // Return null if no matching products
                })}
              </ScrollView>
            )}
          </View>
