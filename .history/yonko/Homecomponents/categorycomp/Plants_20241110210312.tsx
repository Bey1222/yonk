...
const Plants = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          `${process.env.EXPO_PUBLIC_BASE_URL}/products?category=PLANT`,
          {
            headers: {
    ...
    if (categoryData && Object.keys(storeData).length > 0) {
      ...
      sections.forEach((section, index) => {
        if (section === "Green Thumb Picks") {
          ...
          data[section] = []; // Remove all items for Best Sellers
  ...
  const navigateToBrandScreen = (brand) => {
    // Simplified navigation without items and parameters
    if (brand === "Succulents") {
      navigation.navigate("SucculentsScreen");
    } else if (brand === "Herbs") {
      navigation.navigate("HerbsScreen");
    } else if (brand === "Ferns") {
      navigation.navigate("FernsScreen");
    } else if (brand === "Flowering Plants") {
      navigation.navigate("FloweringPlantsScreen");
    }
  };
  ...
  const handleNavigation = (item, screenName) => {
    ...
    navigation.navigate(screenName, {
      plant,
      plantName: store.name,
      plantImageUri: store.avatar,
      plantLocation: `${store.address.street}, ${store.address.city}, ${store.address.state}, ${store.address.country}`,
    });
  };

  const handleNavigationPlantProducts = (item, screenName) => {
    ...
    if (screenName === "PlantsProducts") {
      console.log("Navigating to PlantProducts with item:", item);
      navigation.navigate(screenName, {
        item,
        plantName: store.name,
        plantImageUri: store.avatar,
                  ...
                  onPress={() => {
                    if (section === "Green Thumb Picks") {
                      navigation.navigate("GreenThumbPicks", {
                        greenThumbPicks: Object.values(storeData),
                      });
                    } else if (section === "Discounts") {
                ...
                {sectionData[section]?.map((item, idx) => {
                  ...
                  return (
                    <TouchableOpacity
                      ...
                      style={styles.card}
                      onPress={() =>
                        handleNavigationPlantProducts(item, "PlantsProducts")
                      }
                    >
                ...
                {sectionData[section]?.map((store, idx) => {
                  ...
                  return (
                    <TouchableOpacity
                      key={idx}
                      style={styles.sectionItem}
                      onPress={() => handleNavigation(store, "MainstorePlants")}
                    >
                      <View style={styles.imageContainer}>
                        <Image
