...
const Food = () => {
  ...
  const navigateToBrandScreen = (brand) => {
    if (brand === "Fast Food") {
      ...
      navigation.navigate("LocalDishesScreen");
    } else if (brand === "Alcohol") {
      navigation.navigate("AlcoholScreen");
    } else if (brand === "Healthy") {
      navigation.navigate("HealthyScreen");
    }
  };
  ...
  const handleNavigationFoodProducts = (item, screenName) => {
    if (!item.isOpen) {
      Alert.alert("Restaurant Closed", "This restaurant is currently closed.");
    }
    ...
    if (screenName === "FoodProducts") {
      navigation.navigate(screenName, {
        ...
        restaurantName: store.name,
        restaurantImageUri: store.avatar,
        restaurantLocation: `${store.address.street}, ${store.address.city}, ${store.address.state}, ${store.address.country}`, // Include the location here
      });
    }
  };
                ...
                {sectionData[section]?.map((item, idx) => {
                  ...
                  return (
                    <TouchableOpacity
                      ...
                      style={styles.card}
                      onPress={() =>
                        handleNavigationFoodProducts(item, "FoodProducts")
                      }
                    >
