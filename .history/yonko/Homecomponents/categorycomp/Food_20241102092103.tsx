...
const Food = () => {
  ...
  useEffect(() => {
    ...
    const fetchStores = async () => {
      try {
        const response = await fetch(
          `${process.env.EXPO_PUBLIC_BASE_URL}/shops?subscription=`,
          {
            headers: {
              Authorization: `Bearer ${process.env.EXPO_PUBLIC_TOKEN}`,
            },
  ...
  const getStoreInfo = (shopId) => {
    return storeData.find((store) => store.id === shopId);
  };
            ...
            {section === "Discounts" ? (
              <View style={styles.listContainer}>
                {sortItems(sectionData[section]).map((item, idx) => {
                  const store = getStoreInfo(item.shopId);
                  return (
                    <TouchableOpacity
                      key={idx}
                      style={styles.card}
