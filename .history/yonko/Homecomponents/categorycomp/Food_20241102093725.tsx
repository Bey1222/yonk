...
const Food = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (data && data.products) {
          ...
          await fetchStoresForProducts(data.products);
        } else {
  ...
  const fetchStoresForProducts = async (products) => {
    const storePromises = products.map(async (product) => {
      try {
        const response = await fetch(
          `${process.env.EXPO_PUBLIC_BASE_URL}/shops/${product.shopId}`,
          {
            headers: {
              Authorization: `Bearer ${process.env.EXPO_PUBLIC_TOKEN}`,
            },
        ...
        if (data && data.shop) {
          setStoreData((prevStoreData) => [...prevStoreData, data.shop]);
        } else {
  ...
  useEffect(() => {
    if (categoryData && storeData.length > 0) {
