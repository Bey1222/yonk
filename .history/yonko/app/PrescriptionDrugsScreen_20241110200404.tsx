...
const PrescriptionDrugsScreen: React.FC = () => {
    ...
    const fetchDiscountItems = async () => {
      try {
        const response = await fetch(
          `${process.env.EXPO_PUBLIC_BASE_URL}/products?category=PRESCRIPTION_DRUGS`,
          {
            headers: {
  ...
  const handleNavigationPrescriptionProducts = (
    item: DiscountItem,
    screenName: string
  ) => {
    const store = storeData[item.shopId];

    if (screenName === "PrescriptionProducts") {
      navigation.navigate(screenName, {
        item,
        prescriptionName: store.name,
        prescriptionImageUri: store.avatar,
        prescriptionLocation: `${store.address.street}, ${store.address.city}, ${store.address.state}, ${store.address.country}`, // Include the location here
      });
    }
  };
  ...
  const renderItem = ({
    ...
    <TouchableOpacity
      style={styles.card}
      onPress={() =>
        handleNavigationPrescriptionProducts(item, "PrescriptionProducts")
      }
    >
...
export default PrescriptionDrugsScreen;
