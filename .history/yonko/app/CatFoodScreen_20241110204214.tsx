...
const CatFoodScreen: React.FC = () => {
    ...
    const fetchDiscountItems = async () => {
      try {
        const response = await fetch(
          `${process.env.EXPO_PUBLIC_BASE_URL}/products?category=CAT_FOOD`,
          {
            headers: {
  ...
  const handleNavigationCatFoodProducts = (
    item: DiscountItem,
    screenName: string
  ) => {
  ...
  const renderItem = ({
    ...
    <TouchableOpacity
      style={styles.card}
      onPress={() =>
        handleNavigationCatFoodProducts(item, "PetSuppliesProducts")
      }
    >
