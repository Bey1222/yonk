...
const NewarrivalsSpaBeauty = () => {
    ...
    const fetchNewItems = async () => {
      try {
        const response = await fetch(
          `${process.env.EXPO_PUBLIC_BASE_URL}/products?category=SPA`,
          {
            headers: {
  ...
  const handleNavigationSpaProducts = (item, screenName) => {
    ...
    if (screenName === "SpaBeautyProducts") {
      navigation.navigate(screenName, {
        ...
        spaLocation: `${store.address.street}, ${store.address.city}, ${store.address.state}, ${store.address.country}`, // Include the location here
      });
    }
  };
  ...
  const renderItem = ({ item, index }) => (
    <TouchableOpacity
      style={styles.card}
      onPress={() => handleNavigationSpaProducts(item, "SpaBeautyProducts")}
    >
  ...
  return (
    <LinearGradient
      colors={["#737373", "rgba(241,175,189,1)", "#737373"]}
      locations={[1, 0.2, 1]}
      start={{ x: 0.5, y: 0 }}
      end={{ x: 0, y: 0.7 }}
...
export default NewarrivalsSpaBeauty;
