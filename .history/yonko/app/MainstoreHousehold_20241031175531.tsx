...
const MainstoreHousehold: React.FC<{
  ...
  console.log(household); // Log the household object
  const navigation = useNavigation();
  const [showFullDescription, setShowFullDescription] = useState(false);
  const [searchKeyword, setSearchKeyword] = useState("");
  const [filteredMenu, setFilteredMenu] = useState(household.menu);
  ...
  const handleMenuItemPress = (item: any) => {
    navigation.navigate("HouseholdProducts", {
      ...
      householdImageUri: household.image,
      householdLocation: household.location, // Include the location here
    });
  };
  ...
  const shortDescription = household.about
    ? household.about.split(" ").slice(0, 30).join(" ") + "..."
    : "No description available.";

  const handleSearchChange = (text: string) => {
    setSearchKeyword(text);
    const filtered = household.menu.filter((item) =>
      ...
      >
        <View style={styles.container}>
          ...
          <View style={styles.deliveryTimeContainer}>
            ...
            <Text style={styles.deliveryTime}>{household.deliveryTime}</Text>
          </View>

          <View style={styles.ratingContainer}>
            <Image source={starIcon} style={styles.starIcon} />
            <Text style={styles.ratingText}>{household.rating}</Text>
          </View>
          <View style={styles.locationContainer}>
            <Image source={locationIcon} style={styles.locationIcon} />
            <Text style={styles.location}>{household.location}</Text>
          </View>
                ...
                <View style={styles.aboutCard}>
                  <View style={styles.aboutCardContent}>
                    <Text style={styles.about}>
                      {showFullDescription ? household.about : shortDescription}
                    </Text>
                    <TouchableOpacity
                      onPress={() =>
