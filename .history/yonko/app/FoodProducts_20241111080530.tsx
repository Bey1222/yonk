...
const FoodProducts = ({ route }) => {
  ...
  const handleAddToBasket = async () => {
    ...
    const basketItem = {
      id: item._id,
      name: item._id, // Use item._id instead of item.name
      price: basketPrice,
      sizes: selectedSizes,
    ...
    let totalAmount = 0.0; // Initialize as a float
    let totalQuantity = 0; // Initialize total quantity

    // Log input values
    console.log("Selected Sizes:", JSON.stringify(selectedSizes, null, 2));
    console.log("Selected Add-Ons:", JSON.stringify(selectedAddOns, null, 2));

    // Calculate total amount and quantity
    selectedSizes.forEach((size) => {
      console.log("Processing size:", JSON.stringify(size, null, 2));
      if (typeof size.price !== "number" || typeof size.quantity !== "number") {
        console.error("Invalid size price or quantity:", size);
        return;
      }

      const sizePrice = parseFloat((size.price * size.quantity).toFixed(2));
      console.log(
        `Size: ${size.name}, Price: ${sizePrice.toFixed(2)}, Quantity: ${
          size.quantity
        }`
      );
      totalAmount += sizePrice;
      totalQuantity += size.quantity; // Accumulate total quantity

      if (selectedAddOns[size.name]) {
        selectedAddOns[size.name].forEach((addOn) => {
          console.log("Processing add-on:", JSON.stringify(addOn, null, 2));
          if (typeof addOn.price !== "number") {
            console.error("Invalid add-on price:", addOn);
            return;
          }

          const addOnPrice = parseFloat(
            (addOn.price * size.quantity).toFixed(2)
          );
          console.log(
            `Add-on: ${addOn.name}, Price: ${addOnPrice.toFixed(
              2
    ...
    // Ensure totalAmount is a number with decimal precision
    if (isNaN(totalAmount)) {
      console.error("Total amount calculation resulted in NaN");
      return;
    }

    // Format totalAmount to two decimal places
    totalAmount = parseFloat(totalAmount.toFixed(2));
    console.log(`Total Amount after formatting: ${totalAmount}`);

    // Prepare the request payload
    const requestPayload = {
      productId: item._id,
      quantity: totalQuantity, // Ensure totalQuantity is included
      selectedVariantId: selectedSizes.map((size) => size.variantId).join(", "), // Ensure variantId is included
      selectedAddOns: Object.values(selectedAddOns)
        .flat()
        .map((addOn) => ({
          addOnId: addOn._id, // Ensure addOnId is included
          quantity: addOn.quantity,
        })),
      totalAmount: totalAmount, // Ensure totalAmount is a number with decimal precision
    };
  ...
  const handleSizePress = (size) => {
    setSelectedSizes((prevSelected) => {
      ...
      console.log("Selected Sizes:", newSelectedSizes); // Log selected sizes
    ...
    // Log the selected variant value and base price
    console.log("Selected Variant Value:", size.value);
    console.log("Base Price:", size.price);
  };
                ...
                <TouchableOpacity
                  ...
                  onPress={() =>
                    handleSizePress({
                      ...
                      _id: variantValue._id, // Ensure variantId is included
                    })
                  }
                >
                  <Text
