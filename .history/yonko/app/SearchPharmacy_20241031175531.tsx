...
import PharmacyData from "../assets/data/drugsdata.json";
...
const SearchPharmacy = () => {
  ...
  const filteredPharmacies = useMemo(() => {
    let filtered = PharmacyData.drugs;

    if (activeFilter === "State" && selectedState) {
      filtered = filtered.filter(
        (pharmacy) => pharmacy.state === selectedState
      );
    ...
    // Sort the filtered pharmacies by package: Elite, Premium, Basic
    filtered.sort((a, b) => {
  ...
  const renderItem = ({ item }) => (
    <TouchableOpacity
      style={styles.card}
      onPress={() =>
        navigation.navigate("MainstorePharmacy", { pharmacy: item })
      }
    >
