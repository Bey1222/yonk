...
const FoodProducts = ({ route }) => {
    ...
    setSelectedSizes((prevSelected) => {
      ...
      if (index === -1) {
        newSelectedSizes.push({
          ...size,
          quantity: 1,
          price: size.options?.[0]?.optionValues?.[0]?.price || 0, // Access the nested price
        });
      } else {
  ...
  const renderVariants = () => {
    ...
    if (
      ...
      !Array.isArray(item.variants[0].variantValues)
    ) {
      return (
    ...
    return item.variants[0].variantValues.map((variantValue, vIndex) => (
