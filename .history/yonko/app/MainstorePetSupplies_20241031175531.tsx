...
const MainstorePetSupplies: React.FC<{
  ...
  console.log(petSupply); // Log the petSupply object
  const navigation = useNavigation();
  const [showFullDescription, setShowFullDescription] = useState(false);
  const [searchKeyword, setSearchKeyword] = useState("");
  const [filteredMenu, setFilteredMenu] = useState(petSupply.menu);
  ...
  const handleMenuItemPress = (item: any) => {
    navigation.navigate("PetSuppliesProducts", {
      ...
      petSupplyImageUri: petSupply.image,
      petSupplyLocation: petSupply.location, // Include the location here
    });
  };
  ...
  const shortDescription = petSupply.about
    ? petSupply.about.split(" ").slice(0, 30).join(" ") + "..."
    : "No description available.";

  const handleSearchChange = (text: string) => {
    setSearchKeyword(text);
    const filtered = petSupply.menu.filter((item) =>
      ...
      >
        <View style={styles.container}>
          ...
          <View style={styles.deliveryTimeContainer}>
            ...
            <Text style={styles.deliveryTime}>{petSupply.deliveryTime}</Text>
          </View>

          <View style={styles.ratingContainer}>
            <Image source={starIcon} style={styles.starIcon} />
            <Text style={styles.ratingText}>{petSupply.rating}</Text>
          </View>
          <View style={styles.locationContainer}>
            <Image source={locationIcon} style={styles.locationIcon} />
            <Text style={styles.location}>{petSupply.location}</Text>
          </View>
          ...
          >
            ...
            >
              <View style={styles.nameContainer}>
                <Text style={styles.name}>{petSupply.name}</Text>
                <TouchableOpacity>
                  <Image
                ...
                <View style={styles.aboutCard}>
                  <View style={styles.aboutCardContent}>
                    <Text style={styles.about}>
                      {showFullDescription ? petSupply.about : shortDescription}
                    </Text>
                    <TouchableOpacity
                      onPress={() =>
