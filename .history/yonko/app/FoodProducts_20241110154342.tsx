...
const FoodProducts = ({ route }) => {
    ...
    if (selectedSizes.length === 0) {
      ...
      const requestPayload = {
        ...
        selectedVariantId: selectedSizes
          ...
          .join(", "),
        selectedOptionId: selectedSizes.map((size) => size.optionId).join(", "),
  ...
  // Ensure that selectedVariantId and selectedOptionId are set when selecting sizes
  const handleSizePress = (size) => {
    setSelectedSizes((prevSelected) => {
      const newSelectedSizes = [...prevSelected];
      const index = newSelectedSizes.findIndex((s) => s._id === size._id);
      if (index === -1) {
        newSelectedSizes.push({
          ...
          optionId:
            size.options &&
            size.options.optionValues &&
            size.options.optionValues.length > 0
              ? size.options.optionValues[0]._id
              : null, // Ensure optionId is included, handle cases where optionValues might be undefined
        });
      } else {
