...
const ProteinShakesScreen: React.FC = () => {
    ...
    const fetchDiscountItems = async () => {
      try {
        const response = await fetch(
          `${process.env.EXPO_PUBLIC_BASE_URL}/products?category=PROTEIN_SHAKES`,
          {
            headers: {
  ...
  const handleNavigationProteinShakesProducts = (
    item: DiscountItem,
    screenName: string
  ) => {
    const store = storeData[item.shopId];

    if (screenName === "ProteinShakesProducts") {
      navigation.navigate(screenName, {
        item,
        proteinShakesName: store.name,
        proteinShakesImageUri: store.avatar,
        proteinShakesLocation: `${store.address.street}, ${store.address.city}, ${store.address.state}, ${store.address.country}`, // Include the location here
      });
    }
  };
  ...
  const renderItem = ({
    ...
    <TouchableOpacity
      style={styles.card}
      onPress={() =>
        handleNavigationProteinShakesProducts(item, "ProteinShakesProducts")
      }
    >
...
export default ProteinShakesScreen;
