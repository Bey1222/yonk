...
const DiscountsSpa: React.FC = () => {
  ...
  const handleNavigationSpaProducts = (
    item: DiscountItem,
    screenName: string
  ) => {
    const store = storeData[item.shopId];

    if (screenName === "SpaProducts") {
  ...
  const renderItem = ({
    ...
    <TouchableOpacity
      style={styles.card}
      onPress={() => handleNavigationSpaProducts(item, "SpaProducts")}
    >
...
export default DiscountSpaBeauty;
