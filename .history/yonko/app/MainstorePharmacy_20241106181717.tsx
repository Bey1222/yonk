...
const MainstorePharmacy = ({ route }) => {
  ...
  useEffect(() => {
    ...
    // Log the variants object for each product
    pharmacy.products.forEach((product, index) => {
      console.log(
        `Product ${index + 1} variants:`,
        JSON.stringify(product.variants, null, 2)
      );

      // Log each variant value and its nested options
      if (product.variants && product.variants.variantValues) {
        product.variants.variantValues.forEach((variantValue, variantIndex) => {
          console.log(
            `Product ${index + 1} Variant ${variantIndex + 1}:`,
            JSON.stringify(variantValue, null, 2)
          );

          if (variantValue.options && variantValue.options.optionValues) {
            variantValue.options.optionValues.forEach(
              (optionValue, optionIndex) => {
                console.log(
                  `Product ${index + 1} Variant ${variantIndex + 1} Option ${
                    optionIndex + 1
                  }:`,
                  JSON.stringify(optionValue, null, 2)
                );
              }
            );
          }
