...
const FreshProduceScreen: React.FC = () => {
    ...
    const fetchDiscountItems = async () => {
      try {
        const response = await fetch(
          `${process.env.EXPO_PUBLIC_BASE_URL}/products?category=FRESH_PRODUCE`,
          {
            headers: {
  ...
  const handleNavigationSupernarketProducts = (
    item: DiscountItem,
    screenName: string
  ) => {
    ...
    if (screenName === "SupernarketProducts") {
      navigation.navigate(screenName, {
        ...
        supermarketLocation: `${store.address.street}, ${store.address.city}, ${store.address.state}, ${store.address.country}`, // Include the location here
      });
    }
  };
  ...
  const renderItem = ({
    ...
    <TouchableOpacity
      style={styles.card}
      onPress={() =>
        handleNavigationSupernarketProducts(item, "SupernarketProducts")
      }
    >
  ...
  return (
    <LinearGradient
      colors={["#737373", "rgba(52,18,9,1)", "#737373"]}
      locations={[1, 0.2, 1]}
      start={{ x: 0.5, y: 0 }}
      end={{ x: 0, y: 0.7 }}
...
export default FreshProduceScreen;
