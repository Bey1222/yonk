...
const HomeDecorScreen: React.FC = () => {
    ...
    const fetchDiscountItems = async () => {
      try {
        const response = await fetch(
          `${process.env.EXPO_PUBLIC_BASE_URL}/products?category=HOME_DECOR`,
          {
            headers: {
  ...
  const handleNavigationHouseholdProducts = (
    item: DiscountItem,
    screenName: string
  ) => {
    ...
    if (screenName === "HouseholdProducts") {
      navigation.navigate(screenName, {
        ...
        householdLocation: `${store.address.street}, ${store.address.city}, ${store.address.state}, ${store.address.country}`, // Include the location here
      });
    }
  };
  ...
  const renderItem = ({
    ...
    <TouchableOpacity
      style={styles.card}
      onPress={() =>
        handleNavigationHouseholdProducts(item, "HouseholdProducts")
      }
    >
...
export default HomeDecorScreen;
