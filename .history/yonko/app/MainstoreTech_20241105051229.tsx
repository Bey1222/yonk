...
const MainstoreTech = ({ route }) => {
  const { tech } = route.params;
  const navigation = useNavigation();
  const [showFullDescription, setShowFullDescription] = useState(false);
  const [searchKeyword, setSearchKeyword] = useState("");
  const [filteredMenu, setFilteredMenu] = useState(tech?.products || []);
  ...
  const handleMenuItemPress = (item) => {
    ...
    navigation.navigate("TechProducts", {
      item,
      techName: tech.shop.name,
      techImageUri: tech.shop.avatar,
      techLocation: `${tech.shop.address.street}, ${tech.shop.address.city}, ${tech.shop.address.state}, ${tech.shop.address.country}`,
    });
  };
  ...
  const shortDescription = tech.shop.about
    ? tech.shop.about.split(" ").slice(0, 30).join(" ") + "..."
    : "No description available.";

  const handleSearchChange = (text) => {
    setSearchKeyword(text);
    const filtered = tech.products.filter((item) =>
  ...
  useEffect(() => {
    console.log("MainstoreTech received params:", route.params);
    console.log("Products:", tech.products);
  }, [route.params]);

  if (!tech || !tech.shop || !tech.products) {
    return (
  ...
  return (
    <TouchableWithoutFeedback onPress={handleDismissKeyboard}>
      <LinearGradient
        colors={["#737373", "rgba(12,54,114,.7)", "#737373"]}
        locations={[1, 0.2, 1]}
        start={{ x: 0.5, y: 0 }}
        end={{ x: 0, y: 0.7 }}
      ...
      >
        <View style={styles.container}>
          ...
          <View style={styles.deliveryTimeContainer}>
            ...
            <Text style={styles.deliveryTime}>{tech.shop.deliveryTime}</Text>
          </View>

          <View style={styles.ratingContainer}>
            <Image source={starIcon} style={styles.starIcon} />
            <Text style={styles.ratingText}>{tech.shop.rating}</Text>
          </View>
          <View style={styles.locationContainer}>
            <Image source={locationIcon} style={styles.locationIcon} />
            <Text style={styles.location}>
              {tech.shop.address.street}, {tech.shop.address.city},{" "}
              {tech.shop.address.state}, {tech.shop.address.country}
            </Text>
          </View>
          ...
          >
            ...
            >
              <View style={styles.nameContainer}>
                <Text style={styles.name}>{tech.shop.name}</Text>
                <TouchableOpacity>
                  <Image
                ...
                <View style={styles.aboutCard}>
                  <View style={styles.aboutCardContent}>
                    <Text style={styles.about}>
                      {showFullDescription ? tech.shop.about : shortDescription}
                    </Text>
                    <TouchableOpacity
                      onPress={() =>
