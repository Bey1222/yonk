...
interface NewarrivalsPetSuppliesProps {
  newItems: NewItem[];
}

const NewarrivalsPetSupplies: React.FC = () => {
  const route = useRoute();
  const navigation = useNavigation();
  const { colors } = useTheme();
  const { newItems } = route.params as NewarrivalsPetSuppliesProps;
  ...
  const renderItem = ({ item, index }: { item: NewItem; index: number }) => (
    <TouchableOpacity
      style={styles.card}
      onPress={() =>
        navigation.navigate("PetSuppliesProducts", {
          item,
          petSupplyName: item.parentObj.name,
          petSupplyImageUri: item.parentObj.image,
          petSupplyLocation: item.parentObj.location,
        })
      }
    >
