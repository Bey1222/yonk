...
const SearchPetSupplies = () => {
  ...
  const filteredPetSupplies = useMemo(() => {
    let filtered = petData.pet;

    if (activeFilter === "Category" && selectedCategory) {
      filtered = filtered.filter(
        (petSupply) => petSupply.category === selectedCategory
      );
    } else if (activeFilter === "Brand" && selectedBrand) {
      filtered = filtered.filter((petSupply) =>
        petSupply.menu.some((item) => item.brand === selectedBrand)
      );
    } else if (activeFilter === "Ratings" && selectedRating) {
      filtered = filtered.filter(
        (petSupply) =>
          petSupply.rating >= selectedRating.min &&
          petSupply.rating <= selectedRating.max
      );
    ...
    // Sort the filtered pet supplies based on the package attribute
    filtered.sort((a, b) => {
  ...
  useEffect(() => {
    setResultsCount(filteredPetSupplies.length);
  }, [filteredPetSupplies]);
...
export default SearchPetSupplies;
