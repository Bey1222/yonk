...
const NewArrivalsWardrobe = () => {
    ...
    const fetchNewItems = async () => {
      try {
        const response = await fetch(
          `${process.env.EXPO_PUBLIC_BASE_URL}/products?category=CLOTHING`,
          {
            headers: {
  ...
  const handleNavigationWardrobeProducts = (item, screenName) => {
    const store = storeData[item.shopId];

    if (screenName === "WardrobeProducts") {
      navigation.navigate(screenName, {
        item,
        wardrobeName: store.name,
        wardrobeImageUri: store.avatar,
        wardrobeLocation: `${store.address.street}, ${store.address.city}, ${store.address.state}, ${store.address.country}`, // Include the location here
      });
    }
  };
  ...
  const renderItem = ({ item, index }) => (
    <TouchableOpacity
      style={styles.card}
      onPress={() => handleNavigationWardrobeProducts(item, "WardrobeProducts")}
    >
  ...
  return (
    <LinearGradient
      colors={["#737373", "rgba(156,114,74,1)", "#737373"]}
      locations={[1, 0.2, 1]}
      start={{ x: 0.5, y: 0 }}
      end={{ x: 0, y: 0.7 }}
...
export default NewArrivalsWardrobe;
