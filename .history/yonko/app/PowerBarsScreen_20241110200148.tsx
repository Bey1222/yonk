...
const PowerBarsScreen: React.FC = () => {
    ...
    const fetchDiscountItems = async () => {
      try {
        const response = await fetch(
          `${process.env.EXPO_PUBLIC_BASE_URL}/products?category=POWER_BARS`,
          {
            headers: {
  ...
  const handleNavigationPowerBarsProducts = (
    item: DiscountItem,
    screenName: string
  ) => {
    const store = storeData[item.shopId];

    if (screenName === "PowerBarsProducts") {
      navigation.navigate(screenName, {
        item,
        powerBarsName: store.name,
        powerBarsImageUri: store.avatar,
        powerBarsLocation: `${store.address.street}, ${store.address.city}, ${store.address.state}, ${store.address.country}`, // Include the location here
      });
    }
  };
  ...
  const renderItem = ({
    ...
    <TouchableOpacity
      style={styles.card}
      onPress={() =>
        handleNavigationPowerBarsProducts(item, "PowerBarsProducts")
      }
    >
...
export default PowerBarsScreen;
