...
const MainstorePlants: React.FC<{
  route: { params: { plant: Plant } };
}> = ({ route }) => {
  const { plant } = route.params;
  console.log(plant); // Log the plant object
  const navigation = useNavigation();
  const [showFullDescription, setShowFullDescription] = useState(false);
  const [searchKeyword, setSearchKeyword] = useState("");
  const [filteredMenu, setFilteredMenu] = useState(plant.menu);
  ...
  const handleMenuItemPress = (item: any) => {
    navigation.navigate("PlantsProducts", {
      ...
      plantImageUri: plant.image,
      plantLocation: plant.location, // Include the location here
    });
  };
  ...
  const shortDescription = plant.about
    ? plant.about.split(" ").slice(0, 30).join(" ") + "..."
    : "No description available.";

  const handleSearchChange = (text: string) => {
    setSearchKeyword(text);
    const filtered = plant.menu.filter((item) =>
      ...
      >
        <View style={styles.container}>
          ...
          <View style={styles.deliveryTimeContainer}>
            ...
            <Text style={styles.deliveryTime}>{plant.deliveryTime}</Text>
          </View>
                ...
                <View style={styles.aboutCard}>
                  <View style={styles.aboutCardContent}>
                    <Text style={styles.about}>
                      {showFullDescription ? plant.about : shortDescription}
                    </Text>
                    <TouchableOpacity
                      onPress={() =>
