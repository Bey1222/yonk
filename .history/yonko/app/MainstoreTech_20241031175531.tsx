...
const MainstoreTech: React.FC<{
  route: { params: { tech: Tech } };
}> = ({ route }) => {
  const { tech } = route.params;
  console.log(tech); // Log the tech object
  const navigation = useNavigation();
  const [showFullDescription, setShowFullDescription] = useState(false);
  const [searchKeyword, setSearchKeyword] = useState("");
  const [filteredMenu, setFilteredMenu] = useState(tech.menu);
  ...
  const handleMenuItemPress = (item: any) => {
    navigation.navigate("TechProducts", {
      item,
      techName: tech.name,
      techImageUri: tech.image,
      techLocation: tech.location, // Include the location here
    });
  };
  ...
  const shortDescription = tech.about
    ? tech.about.split(" ").slice(0, 30).join(" ") + "..."
    : "No description available.";

  const handleSearchChange = (text: string) => {
    setSearchKeyword(text);
    const filtered = tech.menu.filter((item) =>
        ...
        <View style={styles.container}>
          ...
          <View style={styles.deliveryTimeContainer}>
            <MaterialCommunityIcons
              name="laptop" // Use a tech-related icon
              size={24}
              color="black"
            />
            <Text style={styles.deliveryTime}>{tech.deliveryTime}</Text>
          </View>
                ...
                <View style={styles.aboutCard}>
                  <View style={styles.aboutCardContent}>
                    <Text style={styles.about}>
                      {showFullDescription ? tech.about : shortDescription}
                    </Text>
                    <TouchableOpacity
                      onPress={() =>
