...
const MainstorePharmacy = ({ route }) => {
  ...
  useEffect(() => {
    ...
    console.log("Products:", JSON.stringify(pharmacy.products, null, 2));

    // Log the entire product object for each product
    pharmacy.products.forEach((product, index) => {
      ...
      // Log each variant value for detailed inspection
      if (product.variants && product.variants.variantValues) {
        product.variants.variantValues.forEach((variantValue, vIndex) => {
          console.log(
            `Product ${index + 1} variant value ${vIndex + 1}:`,
