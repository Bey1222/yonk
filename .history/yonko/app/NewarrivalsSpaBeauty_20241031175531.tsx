...
interface NewarrivalsSpaBeautyProps {
  newItems: NewItem[];
}

const NewarrivalsSpaBeauty: React.FC = () => {
  const route = useRoute();
  const navigation = useNavigation();
  const { colors } = useTheme();
  const { newItems } = route.params as NewarrivalsSpaBeautyProps;
    ...
    <TouchableOpacity
      ...
      onPress={() =>
        navigation.navigate("SpaBeautyProducts", {
          item,
          spaName: item.parentObj.name,
          spaImageUri: item.parentObj.image,
          spaLocation: item.parentObj.location,
        })
      }
    >
