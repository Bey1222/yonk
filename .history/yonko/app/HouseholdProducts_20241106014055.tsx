...
const HouseholdProducts = ({ route }) => {
  ...
  if (item.variants && item.variants.length > 0) {
    item.variants.forEach((variant, index) => {
      console.log(`Received variant ${index + 1}:`, variant);
      if (variant.variantValues && variant.variantValues.length > 0) {
        variant.variantValues.forEach((variantValue, valueIndex) => {
          console.log(
            `Received variant value ${valueIndex + 1}:`,
            variantValue
          );
        });
      }
    ...
    console.log("No variants available for the item.");
  }
