...
const SupernarketProducts = ({ route }) => {
  const {
    item = {},
    supermarketName = "",
    supermarketImageUri = "",
  ...
  console.log("Received supermarketName:", supermarketName);
  console.log("Received supermarketImageUri:", supermarketImageUri);
  console.log("Received supermarketLocation:", supermarketLocation);
  ...
  if (
    ...
    console.error(
      "Missing required parameters in SupermarketProducts component"
    );
    return (
  ...
  const [showAddOnMessage, setShowAddOnMessage] = useState(false); // Add this line
  ...
  const renderPagination = () => {
    return (
      <View style={styles.paginationContainer}>
        {[item.image, ...item.imageGallery].map((_, i) => {
  ...
  const handleAddToBasket = () => {
    if (selectedSizes.length === 0) {
      ...
      const basketItem = {
        ...
        sizes: selectedSizes,
        note,
        restaurant: supermarketName, // Add the supermarket name
        restaurantImageUri: supermarketImageUri, // Use the passed supermarketImageUri
        restaurantLocation: supermarketLocation, // Add the supermarket location
      };
  ...
  const handleFavoritePress = () => {
    ...
    addToWishlist({
      ...
      image: item.image,
      sizes: selectedSizes,
      restaurantName: supermarketName, // Add the supermarket name
      restaurantImageUri: supermarketImageUri, // Add the supermarket image URI
      restaurantLocation: supermarketLocation, // Add the supermarket location
    });
    ...
    >
      <ScrollView showsVerticalScrollIndicator={false}>
        <View style={styles.swiperContainer}>
          ...
          >
            {[item.image, ...item.imageGallery].map((image, index) => (
            ...
            <View style={styles.sizesContainer}>
              {sizes.map((size) => (
                ...
                >
                  ...
                  >
                    {size.name} (N{size.price.toFixed(2)})
                  </Text>
                </TouchableOpacity>
              ))}
            </View>
      ...
      <View style={styles.bottomNavbar}>
        ...
        >
          ...
          <Text style={styles.priceText}>N{basketPrice.toFixed(2)}</Text>
        </TouchableOpacity>
      </View>
      ...
      {showAddedToWishlistMessage && (
        <Animated.View
          style={[
            styles.addedToBasketMessage, // Use the same style as addedToBasketMessage
            {
              transform: [{ translateY: animation }],
            },
          ]}
...
export default SupernarketProducts;
