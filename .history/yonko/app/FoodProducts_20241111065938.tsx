  ...
  const handleAddToBasket = async () => {
    if (selectedSizes.length === 0) {
      ...
      selectedSizes.forEach((size) => {
        ...
        console.log(
          `Size: ${size.name}, Price: ${sizePrice}, Quantity: ${size.quantity}`
        );
        ...
        if (selectedAddOns[size.name]) {
          selectedAddOns[size.name].forEach((addOn) => {
            ...
            const addOnPrice = addOn.price * size.quantity;
            console.log(
              `Add-on: ${addOn.name}, Price: ${addOnPrice}, Quantity: ${size.quantity}`
            );
