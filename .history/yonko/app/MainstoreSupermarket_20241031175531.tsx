...
const MainstoreSupermarket: React.FC<{
  route: { params: { supermarket: Supermarket } };
}> = ({ route }) => {
  const { supermarket } = route.params;
  console.log(supermarket); // Log the supermarket object
  const navigation = useNavigation();
  const [showFullDescription, setShowFullDescription] = useState(false);
  const [searchKeyword, setSearchKeyword] = useState("");
  const [filteredMenu, setFilteredMenu] = useState(supermarket.menu);
  ...
  const handleMenuItemPress = (item: any) => {
    navigation.navigate("SupernarketProducts", {
      ...
      supermarketImageUri: supermarket.image,
      supermarketLocation: supermarket.location, // Include the location here
    });
  };
  ...
  const shortDescription = supermarket.about
    ? supermarket.about.split(" ").slice(0, 30).join(" ") + "..."
    : "No description available.";

  const handleSearchChange = (text: string) => {
    setSearchKeyword(text);
    const filtered = supermarket.menu.filter((item) =>
      ...
      >
        <View style={styles.container}>
          ...
          <View style={styles.deliveryTimeContainer}>
            ...
            <Text style={styles.deliveryTime}>{supermarket.deliveryTime}</Text>
          </View>

          <View style={styles.ratingContainer}>
            <Image source={starIcon} style={styles.starIcon} />
            <Text style={styles.ratingText}>{supermarket.rating}</Text>
          </View>
          <View style={styles.locationContainer}>
            <Image source={locationIcon} style={styles.locationIcon} />
            <Text style={styles.location}>{supermarket.location}</Text>
          </View>
