...
const DogFoodScreen: React.FC = () => {
    ...
    const fetchDiscountItems = async () => {
      try {
        const response = await fetch(
          `${process.env.EXPO_PUBLIC_BASE_URL}/products?category=DOG_FOOD`,
          {
            headers: {
  ...
  const handleNavigationDogFoodProducts = (
    item: DiscountItem,
    screenName: string
  ) => {
  ...
  const renderItem = ({
    ...
    <TouchableOpacity
      style={styles.card}
      onPress={() =>
        handleNavigationDogFoodProducts(item, "PetSuppliesProducts")
      }
    >
