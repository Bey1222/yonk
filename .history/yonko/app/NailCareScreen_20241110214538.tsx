...
const NailCareScreen: React.FC = () => {
    ...
    const fetchDiscountItems = async () => {
      try {
        const response = await fetch(
          `${process.env.EXPO_PUBLIC_BASE_URL}/products?category=NAIL_CARE`,
          {
            headers: {
  ...
  const handleNavigationSpaBeautyProducts = (
    item: DiscountItem,
    screenName: string
  ) => {
  ...
  const renderItem = ({
    ...
    <TouchableOpacity
      style={styles.card}
      onPress={() =>
        handleNavigationSpaBeautyProducts(item, "SpaBeautyProducts")
      }
    >
