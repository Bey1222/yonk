...
const FoodProducts = ({ route }) => {
  ...
  const renderVariants = () => {
    ...
    return parsedVariants.map((variant, vIndex) => (
      <View key={vIndex} style={styles.variantValueContainer}>
        ...
        variant.variantValues.length > 0 ? (
          variant.variantValues.map((variantValue, vvIndex) => (
            <View key={vvIndex} style={styles.variantValueContainer}>
              <Text style={[styles.variantValue, { color: "#fff" }]}>
                {variantValue.value}
              </Text>
        ...
        <View style={styles.detailsContainer}>
          ...
          <View style={styles.sizesContainer}>
            {parsedVariants && parsedVariants.length > 0 ? (
              parsedVariants.map((variant, vIndex) => (
                <TouchableOpacity
                  key={vIndex}
                  style={[
                    styles.sizeButton,
                  ...
                  onPress={() =>
                    handleSizePress({
                      ...
                      price:
                        variant.variantValues?.[0]?.options?.[0]
                          ?.optionValues?.[0]?.price || 0, // Access the nested price
                    })
                  }
                >
                  <Text
                  ...
                  >
                    {variant.variantName} (₦
                    {variant.variantValues?.[0]?.options?.[0]?.optionValues?.[0]
                      ?.price
                      ? variant.variantValues?.[0]?.options?.[0]?.optionValues?.[0]?.price.toFixed(
                          2
                        )
                      : "0.00"}
                    )
