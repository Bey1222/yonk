...
const NewArrivalsSupermarket = () => {
    ...
    const fetchNewItems = async () => {
      try {
        const response = await fetch(
          `${process.env.EXPO_PUBLIC_BASE_URL}/products?category=GROCERY`,
          {
            headers: {
  ...
  const handleNavigationSupermarketProducts = (item, screenName) => {
    ...
    if (screenName === "SupermarketProducts") {
  ...
  const renderItem = ({ item, index }) => (
    <TouchableOpacity
      style={styles.card}
      onPress={() =>
        handleNavigationSupermarketProducts(item, "SupermarketProducts")
      }
    >
...
export default NewarrivalsSupermarket;
