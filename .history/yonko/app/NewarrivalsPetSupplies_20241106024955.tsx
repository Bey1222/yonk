...
const NewarrivalsPetSupplies = () => {
  ...
  const handleNavigationPetSuppliesProducts = (item, screenName) => {
    ...
    if (screenName === "PetSuppliesProducts") {
      navigation.navigate(screenName, {
        ...
        petSupplyLocation: `${store.address.street}, ${store.address.city}, ${store.address.state}, ${store.address.country}`, // Include the location here
      });
    }
  };
  ...
  const renderItem = ({ item, index }) => (
    <TouchableOpacity
      style={styles.card}
      onPress={() =>
        handleNavigationPetSuppliesProducts(item, "PetSuppliesProducts")
      }
    >
  ...
  return (
    <LinearGradient
      colors={["#737373", "rgba(255, 240, 245, .8)", "#737373"]}
      locations={[1, 0.2, 1]}
      start={{ x: 0.5, y: 0 }}
      end={{ x: 0, y: 0.7 }}
...
export default NewarrivalsPetSupplies;
