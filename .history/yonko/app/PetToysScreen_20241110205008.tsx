...
const PetToysScreen: React.FC = () => {
    ...
    const fetchDiscountItems = async () => {
      try {
        const response = await fetch(
          `${process.env.EXPO_PUBLIC_BASE_URL}/products?category=PET_TOYS`,
          {
            headers: {
  ...
  const handleNavigationPetToysProducts = (
    item: DiscountItem,
    screenName: string
  ) => {
  ...
  const renderItem = ({
    ...
    <TouchableOpacity
      style={styles.card}
      onPress={() =>
        handleNavigationPetToysProducts(item, "PetSuppliesProducts")
      }
    >
