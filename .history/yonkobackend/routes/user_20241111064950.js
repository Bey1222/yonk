...
router.post('/cart/addToCart', authenticateToken, async (req, res) => {
    try {
        const { productId, quantity, selectedVariant, selectedOption, selectedAddOns, price, totalAmount } = req.body;
        const token = req.headers.authorization; // Assuming the token is sent in the Authorization header
...
router.post('/cart/reduceQuantity', authenticateToken, async (req, res) => {
    try {
        const { productId, quantity, selectedVariant, selectedOption, selectedAddOns, price } = req.body;
        const token = req.headers.authorization; // Assuming the token is sent in the Authorization header
